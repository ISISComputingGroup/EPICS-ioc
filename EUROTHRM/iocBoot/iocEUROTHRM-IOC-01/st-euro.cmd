...
## Load ReadASCII
## A seperate instance must be created for each Eurothrm
$(IFACTIVE) ReadASCIIConfigure("READASCII$(EURO_NUM)", "$(RAMP_DIR)", 1, 20)
...
$(IFACTIVE) $(IFNEEDLE_VALVE) dbLoadRecords("$(TOP)/db/automaticNeedleValve.db", "P=$(P),Q=$(MYPVPREFIX)$(IOCNAME):,GAD=$(GAD), LAD=$(LAD), TEMP_SCALING=$(TEMP_SCALING_$(EURO_NUM)=1.0),FLOW_SCALING=$(FLOW_SCALING_$(EURO_NUM)=1.0),FLOW_TEMP_BL_SCALING=$(FLOW_TEMP_BL_SCALING_$(EURO_NUM)=1.0),FLOW_SCALE_SCALING=$(FLOW_SCALE_SCALING_$(EURO_NUM)=1.0),IFTOGGLE_WRITE=$(IFTOGGLE_WRITE=)")
