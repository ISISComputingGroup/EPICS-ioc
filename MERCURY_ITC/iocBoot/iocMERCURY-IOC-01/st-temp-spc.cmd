...
stringiftest("VTI_SPC_PRESSURE", "$(VTI_SPC_PRESSURE_$(TEMP_NUM)=)")

epicsEnvSet "CALIB_BASE_DIR" "$(ICPCONFIGBASE)/$(INSTRUMENT)"
epicsEnvSet "SENS_DIR" "calib/mercury_vti_spc"
...
$(IFVTI_SPC_PRESSURE) dbLoadRecords("$(MERCURY_ITC)/db/MercuryVTISPC.db", "P=$(MYPVPREFIX)$(IOCNAME):,TEMP_CARD=$(TEMP_NUM),PRESSURE_CARD=$(VTI_SPC_PRESSURE_$(TEMP_NUM)=),MIN_PRESSURE=$(VTI_SPC_MIN_PRESSURE=0.0),MAX_PRESSURE=$(VTI_SPC_MAX_PRESSURE=0.0),PRESSURE_CONSTANT=$(VTI_SPC_PRESSURE_CONSTANT=5.0),PRESSURE_MAX_LKUP=$(VTI_SPC_PRESSURE_MAX_LKUP=None.txt),CALIB_BASE_DIR=$(CALIB_BASE_DIR),SDIR=$(SENS_DIR),TEMP_CUTOFF_POINT=$(VTI_SPC_TEMP_CUTOFF_POINT=5.0),TEMP_SCALE=$(VTI_SPC_TEMP_SCALE=2.0),RECSIM=$(RECSIM),DISABLE=$(DISABLE)")
