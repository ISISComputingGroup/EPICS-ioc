...
$(IFPORT) dbLoadRecords("$(TOP)/db/SDTEST-IOC-01.db","P=$(MYPVPREFIX),Q=$(IOCNAME):P$(PN):,PORT=SD$(PN),DEV=$(PORT$(PN)=),NAME=$(PORT$(PN)_NAME=),DISABLE=$(DISABLE),SIM=$(SIMULATE),SCAN=$(SCAN$(PN)=Passive),SETOUTA=$(SETOUTA$(PN)=),SETOUTB=$(SETOUTB$(PN)=),GETOUT=$(GETOUT$(PN)=),GETIN=$(GETIN$(PN)=),SETFMT=$(SETFMT$(PN)=),SETREPLY=$(SETREPLY$(PN)=)")
