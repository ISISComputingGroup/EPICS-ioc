...
$(IFADR) dbLoadRecords("$(TOP)/db/DFKPS_RIKEN_STATUS.db", "device=$(MYPVPREFIX)$(IOCNAME), P=$(MYPVPREFIX)$(IOCNAME):$(PSUNAME):, port=$(DEVICE), ADR=$(PSUID), powpos=$(PFP$(PS)=$(POWER_FLAG_POSITION)), ilkpos=$(CHAIN$(CHAIN)_ILK$(PS)=$(INTERLOCK_FLAG_POSITION))")

$(IFADR) dbLoadRecords("$(TOP)/db/DFKPS_RIKEN_STATUS_ERRORS.db", "device=$(MYPVPREFIX)$(IOCNAME), P=$(MYPVPREFIX)$(IOCNAME):$(PSUNAME):, port=$(DEVICE), ADR=$(PSUID)")
