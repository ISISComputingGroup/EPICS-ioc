...
$(IFACTIVE) dbLoadRecords("$(TOP)/db/devEurotherm.db","P=$(MYPVPREFIX)${IOCPVPREFIX}:A0$(EURO_NUM):, Q=$(MYPVPREFIX)${IOCPVPREFIX}:, ADDR=$(ADDR_$(EURO_NUM)=0), GAD=$(GAD), LAD=$(LAD), PORT=L0, CALIB_BASE_DIR= $(CALIB_BASE_DIR), SDIR=$(SENS_DIR), READ=READASCII$(EURO_NUM), RAMPLIST=RAMPFILELIST$(EURO_NUM), SENSORLIST=SENSORFILELIST$(EURO_NUM)")
