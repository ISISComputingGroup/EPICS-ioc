...
$(IFACTIVE) FileListConfigure("SENSORFILELIST$(EURO_NUM)", "$(SENS_DIR)", "$(SENS_PAT)", 1) 
...
$(IFACTIVE) dbLoadRecords("$(TOP)/db/devEurotherm.db","P=$(MYPVPREFIX)${IOCPVPREFIX}:A0$(EURO_NUM):, Q=$(MYPVPREFIX)${IOCPVPREFIX}:, GAD=$(GADDR_$(EURO_NUM)=0), LAD=$(LADDR_$(EURO_NUM)=0), PORT=L0, SDIR=$(SENS_DIR), READ=READASCII$(EURO_NUM), RAMPLIST=RAMPFILELIST$(EURO_NUM), SENSORLIST=SENSORFILELIST$(EURO_NUM)")
